...
let modalKey = 0
let primeiroidem= pizzaJson.id=1
...
const pegarKey = (e) => {
    ...
    let keyy = e.target.closest('.pizza-item').getAttribute('data-key')
...
const mudarQuantidade = (key) => {
    // Ações nos botões + e - da janela modal
    seleciona('.pizzaInfo--qtmais').addEventListener('click', (key) => {
 if((seleciona('.pizzaInfo h1').innerHTML=='Aluguel do Espaço')&&(quantPizzas>=1)){
	...
	if(cart.length > 0) {
		...
		for(let i in cart) {
			...
			cartItem.querySelector('.cart--item-qtmais').addEventListener('click', () => {
                ...
                if((pizzaName=='Aluguel do Espaço')&&(cart[i].qt>=1)){
		...
		desconto = "Aluguel do espaço 380"
		total = subtotal 
